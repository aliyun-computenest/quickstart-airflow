...
2.然后设置存放DAG文件的Git仓库配置，本服务采用Git仓库保存DAG文件的方式，这里支持公开和私有
两种仓库，选择私有仓库时，需要生成accessToken当作访问凭证。
![img_4.png](img_4.png)
...
## 常见问题答疑
1.git-sync容器启动失败
> 由于需要访问git仓库，国内机器可能无法访问github，可以选择海外地域进行部署，可以自行对部署的helm release进行升级修改，具体配置方式可以参考
